...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    # Define Loss Function->eval将其中的python语句转化为python语句
    print("Next, we execute: " + 'torch.nn.' + __C.LOSS_FUNC_NAME_DICT[__C.LOSS_FUNC] + "(reduction='" + __C.LOSS_REDUCTION + "').cuda()")
    ...
    if __C.RESUME:
        ...
        if __C.CKPT_PATH is not None:
            ...
            path = __C.CKPTS_PATH + '/ckpt_' + __C.CKPT_VERSION + '/epoch' + str(__C.CKPT_EPOCH) + '.pkl'
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (frcn_feat_iter, grid_feat_iter, bbox_feat_iter, ques_ix_iter, ans_iter) in enumerate(dataloader):
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                sub_frcn_feat_iter = frcn_feat_iter[accu_step * __C.SUB_BATCH_SIZE:(accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_grid_feat_iter = grid_feat_iter[accu_step * __C.SUB_BATCH_SIZE:(accu_step + 1) * __C.SUB_BATCH_SIZE]
